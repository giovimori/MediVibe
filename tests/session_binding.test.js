...
test('Session Binding: Legare la sessione al fingerprint del client (IP e User-Agent)', async () => {
    ...
    console.log(`[DEBUG Session Binding] Valore atteso (Accesso con UA modificato bloccato): 403`);
    console.log(`[DEBUG Session Binding] Valore effettivo: ${resHijackedUA.status}`);
    assert.strictEqual(resHijackedUA.status, 403, "Il cambio di User-Agent deve produrre un HTTP 403 Forbidden.");
    ...
    console.log(`[DEBUG Session Binding] Valore atteso (Accesso con IP modificato bloccato): 403`);
    console.log(`[DEBUG Session Binding] Valore effettivo: ${resHijackedIP.status}`);
    assert.strictEqual(resHijackedIP.status, 403, "Il cambio di IP deve produrre un HTTP 403 Forbidden.");
